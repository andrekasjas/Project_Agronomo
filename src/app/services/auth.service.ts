...
export class AuthService {
  ...
  async resetPassword(email: string): Promise<void> {
    try {
      console.log("entre perfect restablecer");
  ...
  async register(email: string, password: string): Promise<User> {
    try {
      console.log("entre perfect registrar");
      const { user } = await this.afAuth.createUserWithEmailAndPassword(email, password);
      await this.sendVerifcationEmail();
      return user;
    } catch (error) {
      console.log('Ya esta registrado o No cumple las condiciones', error);
    }
  }

  async mibellologin(email: string, password: string): Promise<User> {
    try {
      console.log("entre perfect login");
  ...
  async sendVerifcationEmail(): Promise<void> {
    try {
      console.log("entre perfect verificacion");
 ...
 public updateUserData(user: User) {
  ...
  async loginwithfacebook(): Promise<User>{
    try{
      console.log("entre perfect a facebook");
  ...
  async loginwithgoogle(): Promise<User>{
    try{
      console.log("entre perfect a google");
